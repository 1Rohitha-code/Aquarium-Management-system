...
<!--------------------------------------------------------------->
    <div class="row">
            ...
            <div class="card-body">
              ...
              <tr>
                  ...
                  @foreach($rfq_status as $product)
                              ...
                              <select  name="rfq_status" class="form-select rfq_status" aria-label="Default select example" style="width:120px;height:38px" disabled>
        ...
        <div class="card" style="width:565px;">
            ...
            <form method="post" action="/update_quotation_item_data/<?php echo $quotation_id; ?>/<?php echo $get_rfq_ref_id;?>/<?php echo Auth::user()->id; ?>" enctype="multipart/form-data">
                            @csrf
             ...
             <input type="hidden" name="rfq_date_of_delivered" value="<?php foreach($rfq_data as $val){ echo $val->date_of_delivered;}?>"> 
                        ...
                        <a href="/quotation/<?php echo $quotation_id; ?>/<?php echo $get_rfq_ref_id;?>/<?php echo Auth::user()->id; ?>" class="btn btn-primary selecte_items" style="height:38px;width:150px"> Continue &nbsp;<i class="fa fa-step-forward" aria-hidden="true"></i></a>
                        </div>
                        <br><br>
           ...
           @foreach($retrieve_stored_items as $data)
              ...
              <td>
                  @if($data->required_qty != NULL)
                    <form method="post" action="/update_quotation_item_data/<?php echo $quotation_id; ?>/<?php echo $get_rfq_ref_id;?>/<?php echo Auth::user()->id; ?>" enctype="multipart/form-data">
                                @csrf
                     <input type="hidden" name="tbl_id" value="{{$data->id}}">
                    <input type="text" name="requested_qty" class="form-control" style="width:100px" value="{{$data->required_qty}}" >
                    <button type="submit" class="btn btn-secondary btn-block btn-sm" style="width:100px">Update</button>
                    </form>
                  @else
                    <form method="post" action="/update_quotation_item_data/<?php echo $quotation_id; ?>/<?php echo $get_rfq_ref_id;?>/<?php echo Auth::user()->id; ?>" enctype="multipart/form-data">
                                @csrf
                    <input type="hidden" name="tbl_id" value="{{$data->id}}">
                    <input type="text"  name="requested_qty" style="background-color:white" style="width:100px" class="form-control" value="{{$data->required_qty}}" placeholder="enter Qty">
                    <button type="submit" class="btn btn-secondary btn-block btn-sm" style="width:100px">Update  </button>
                    </form>
                    @endif
...
<!----------used to alert num of input fields---------------------------------------------> 
<!-- <script>
          var inputFormDiv = document.getElementById('inputForm');
          if(inputFormDiv.getElementsByTagName('input').length == 3){
